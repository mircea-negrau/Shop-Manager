...
class DeletedSheetsApi {
  ...
  static Future<void> init() async {
    try {
      ...
      _deletedSheet = await _getWorkSheet(spreadsheet, title: 'Deleted');
      final firstRow = OrderFields.getFields();
      _deletedSheet!.values.insertRow(1, firstRow);
    } catch (e) {
      print('Init error: $e');
    }
  }
  ...
  static Future<Order?> getById(int id) async {
    if (_deletedSheet == null) return null;
    final json = await _deletedSheet!.values.map.rowByKey(id, fromColumn: 1);
  ...
  static Future<int> _getRowCount() async {
    if (_deletedSheet == null) return 0;
    final lastRow = await _deletedSheet!.values.lastRow();
  ...
  static Future insert(List<Map<String, dynamic>> rowList) async {
    if (_deletedSheet == null) return;
    _deletedSheet!.values.map.appendRows(rowList);
  }

  static Future<List<Order>> getAll() async {
    if (_deletedSheet == null) return <Order>[];
    final orders = await _deletedSheet!.values.map.allRows();
    return orders == null ? <Order>[] : orders.map(Order.fromJson).toList();
  }

  static Future<bool> update(int id, Map<String, dynamic> order) async {
    if (_deletedSheet == null) return false;
    return _deletedSheet!.values.map.insertRowByKey(id, order);
  }

  static Future<bool> deleteById(int id) async {
    if (_deletedSheet == null) return false;
    final index = await _deletedSheet!.values.rowIndexOf(id);
    if (index == -1) return false;
    return _deletedSheet!.deleteRow(index);
  }
}
