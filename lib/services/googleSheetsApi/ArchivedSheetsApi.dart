...
class ArchivedSheetsApi {
  ...
  static Future<void> init() async {
    try {
      ...
      _archivedSheet = await _getWorkSheet(spreadsheet, title: 'Archived');
      final firstRow = OrderFields.getFields();
      _archivedSheet!.values.insertRow(1, firstRow);
    } catch (e) {
      print('Init error: $e');
    }
  }
  ...
  static Future<Order?> getById(int id) async {
    if (_archivedSheet == null) return null;
    final json = await _archivedSheet!.values.map.rowByKey(id, fromColumn: 1);
  ...
  static Future<int> _getRowCount() async {
    if (_archivedSheet == null) return 0;
    final lastRow = await _archivedSheet!.values.lastRow();
  ...
  static Future insert(List<Map<String, dynamic>> rowList) async {
    if (_archivedSheet == null) return;
    _archivedSheet!.values.map.appendRows(rowList);
  }

  static Future<List<Order>> getAll() async {
    if (_archivedSheet == null) return <Order>[];
    final orders = await _archivedSheet!.values.map.allRows();
    return orders == null ? <Order>[] : orders.map(Order.fromJson).toList();
  }

  static Future<bool> update(int id, Map<String, dynamic> order) async {
    if (_archivedSheet == null) return false;
    return _archivedSheet!.values.map.insertRowByKey(id, order);
  }

  static Future<bool> deleteById(int id) async {
    if (_archivedSheet == null) return false;
    final index = await _archivedSheet!.values.rowIndexOf(id);
    if (index == -1) return false;
    return _archivedSheet!.deleteRow(index);
  }
}
